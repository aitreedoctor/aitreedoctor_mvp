...
class DiagnoseUpdateRequest(BaseModel):
    ...
    pesticides: Optional[List[PesticideInfo]] = None

@router.put("/diagnose/{id}", response_model=DiagnoseResponse)
async def update_diagnose(id: str, req: DiagnoseUpdateRequest):
    conn = get_db_connection()
    cursor = conn.cursor()
    
    # Check if exists
    cursor.execute("SELECT id FROM diagnoses WHERE id = ?", (id,))
    ...
    if req.tree_species is not None:
        updates.append("tree_species = ?")
        params.append(req.tree_species)
    if req.suspected_disease is not None:
        updates.append("suspected_disease = ?")
        params.append(req.suspected_disease)
    if req.severity_level is not None:
        updates.append("severity_level = ?")
        params.append(req.severity_level)
    if req.status_leaves is not None:
        updates.append("status_leaves = ?")
        params.append(req.status_leaves)
    if req.status_stems is not None:
        updates.append("status_stems = ?")
        params.append(req.status_stems)
    if req.status_roots is not None:
        updates.append("status_roots = ?")
        params.append(req.status_roots)
    if req.treatment_method is not None:
        updates.append("treatment_method = ?")
        params.append(req.treatment_method)
    if req.immediate_actions is not None:
        updates.append("immediate_actions = ?")
        params.append(json.dumps(req.immediate_actions))
    if req.pesticides is not None:
        p_list = []
        for p in req.pesticides:
            p_list.append({
                "product_name": p.product_name,
                "active_ingredient": p.active_ingredient,
                "dilution_ratio": p.dilution_ratio,
                "safety_standard": p.safety_standard,
                "dosage": p.dosage,
                "prescription_days": p.prescription_days
            })
        updates.append("pesticide_prescription = ?")
        params.append(json.dumps(p_list))
        
    if updates:
        query = f"UPDATE diagnoses SET {', '.join(updates)} WHERE id = ?"
        params.append(id)
